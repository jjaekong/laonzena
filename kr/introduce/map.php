<?php require_once($_SERVER['DOCUMENT_ROOT'].'/kr/inc/dochead.php'); ?>
</head>
<body class="introduce map">
    <?php require_once($_SERVER['DOCUMENT_ROOT'].'/kr/inc/header.php'); ?>
    <main id="content" tabindex="-1">
        <div class="container">
            <div class="row">
                <div class="col-xs-12 col-lg-5">
                    <div class="content-box">
                        <div class="content-cell">
                            <article class="desc">
                                <header class="desc-head">
                                    <h3 class="desc-title">MAP</h3>
                                </header>
                                <div class="desc-body">
                                    <p>
                                        <strong>成都市锦江区二环路东五段琉璃路8号华润广场5号楼2层</strong>
                                    </p>
                                    <p>
                                        T. 1234567890<br>
                                        F. 1234567890
                                    </p>
                                    <p>
                                        <a href="mailto:Laon-zena@naver.com">Laon-zena@naver.com</a>
                                    </p>
                                </div>
                                <footer class="desc-foot">
                                    <ul class="desc-link">
                                        <li><a href="#">DOWNLOAD BROCHURE</a></li>
                                        <li><a href="/kr/community/reservation.php">RESERVATIONS</a></li>
                                    </ul>
                                </footer>
                            </article>
                        </div>
                    </div>
                </div>
                <div class="col-xs-12 col-lg-7">
                    <div class="content-box">
                        <div class="content-cell">
                            <div class="map-area">
                                <img src="/kr/assets/images/map.jpg">
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </main>
    <?php require_once($_SERVER['DOCUMENT_ROOT'].'/kr/inc/footer.php'); ?>
    <?php require_once($_SERVER['DOCUMENT_ROOT'].'/kr/inc/docfoot.php'); ?>
</body>
</html>
